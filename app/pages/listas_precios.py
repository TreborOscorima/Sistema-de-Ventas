...
def _tab_btn(label: rx.Component, tab: str) -> rx.Component:
    return rx.el.button(
        label,
        on_click=State.set_pl_detail_tab(tab),
        class_name=rx.cond(
            State.pl_detail_tab == tab,
...
def _tab_clientes() -> rx.Component:
    return rx.el.div(
        # Buscador para asignar
        rx.el.div(
            rx.el.p("Asignar cliente", class_name="text-sm font-semibold text-slate-700"),
            rx.el.div(
                rx.debounce_input(
                    rx.input(
                        placeholder="Buscar cliente por nombre...",
                        value=State.pl_client_search,
                        on_change=State.pl_search_clients_to_assign,
                        class_name=INPUT_STYLES["default"],
                    ),
                    debounce_timeout=350,
                ),
                rx.cond(
                    State.pl_client_search_results.length() > 0,
                    rx.el.div(
                        rx.foreach(
                            State.pl_client_search_results,
                            lambda c: rx.el.div(
                                rx.el.div(
                                    rx.el.p(c["name"], class_name="text-sm font-medium text-slate-800"),
                                    rx.cond(
                                        c["email"] != "",
                                        rx.el.p(c["email"], class_name="text-xs text-slate-400"),
                                        rx.fragment(),
                                    ),
                                ...
                                rx.cond(
                                    ...
                                    rx.el.button(
                                        rx.icon("user-plus", class_name="h-3.5 w-3.5 mr-1"),
                                        "Asignar",
                                        on_click=State.pl_assign_client(c["id"]),
                                        class_name="text-xs px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg flex items-center gap-1 whitespace-nowrap",
                                    ),
                                ),
                                class_name="flex items-center justify-between gap-3 px-3 py-2 hover:bg-indigo-50 border-b border-slate-100",
                            ),
                        ),
                        class_name="absolute z-50 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-52 overflow-y-auto",
                    ),
                    rx.fragment(),
                ),
                class_name="relative",
        ...
        rx.cond(
            State.selected_price_list_clients.length() > 0,
            rx.el.div(
                rx.el.p(
                    "Clientes asignados (",
                    State.selected_price_list_clients.length().to_string(),
                    ")",
                    class_name="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2",
                ),
                rx.el.div(
                    rx.foreach(
                        State.selected_price_list_clients,
                        lambda c: rx.el.div(
                            rx.el.div(
                                rx.el.p(c["name"], class_name="text-sm font-medium text-slate-800"),
                                rx.cond(
                            ...
                            rx.el.button(
                                ...
                                on_click=State.pl_remove_client(c["id"]),
                                class_name="p-1.5 text-red-400 hover:bg-red-50 rounded-lg",
                                title="Quitar de la lista",
                            ),
                            class_name="flex items-center justify-between gap-3 py-2 border-b border-slate-100",
                        ),
                    ),
            ...
            rx.el.div(
                rx.icon("users", class_name="h-8 w-8 text-slate-300 mx-auto"),
                rx.el.p(
                    "Sin clientes asignados. Buscá un cliente arriba para asignarlo.",
                    class_name="text-sm text-slate-400 mt-2 text-center",
                ),
                class_name="py-8",
            ),
...
def _price_list_detail_modal() -> rx.Component:
    return modal_container(
        ...
        title=rx.el.div(
            rx.el.span("Lista de precios: ", class_name="text-slate-500 font-normal"),
        ...
        children=[
            ...
            rx.el.div(
                rx.el.div(
                    _tab_btn(
                        rx.el.span(
                            rx.icon("tag", class_name="h-3.5 w-3.5 inline-block mr-1.5"),
                            "Precios (",
                            State.price_list_items.length().to_string(),
                            ")",
                        ),
                        "precios",
                    ),
                    _tab_btn(
                        rx.el.span(
                            rx.icon("users", class_name="h-3.5 w-3.5 inline-block mr-1.5"),
                            "Clientes (",
                            State.selected_price_list_clients.length().to_string(),
                            ")",
                        ),
                    ...
                    class_name="flex border-b border-slate-200",
                ),
                class_name="mb-2",
            ),
            # Contenido de pestaña activa
            rx.cond(
                State.pl_detail_tab == "precios",
                _tab_precios(),
                _tab_clientes(),
            ),
        ],
