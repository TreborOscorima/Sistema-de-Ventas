...
def field_prices_section() -> rx.Component:
  return rx.el.div(
    rx.el.div(
      rx.el.h2(
        "PRECIOS DE CAMPO", class_name="text-xl font-semibold text-slate-700"
      ),
      rx.el.p(
    ...
    rx.el.div(
      ...
      class_name="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr,2fr,1fr,auto,auto] gap-3 items-center bg-white p-4 rounded-xl shadow-sm border",
    ),
    rx.cond(
      State.editing_field_price_id != "",
      rx.el.div(
      ...
      rx.el.table(
        rx.el.thead(
          rx.el.tr(
            ...
            rx.el.th(
              "Precio (por hora)", class_name=TABLE_STYLES["header_cell"]
            ),
            rx.el.th(
              "Acciones", class_name=f"{TABLE_STYLES['header_cell']} text-center"
            ),
            ...
            lambda price: rx.el.tr(
              ...
              rx.el.td(
                rx.el.input(
                  ...
                  class_name="w-full sm:w-32 h-10 px-3 text-sm bg-white border border-slate-200 rounded-md focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500",
                ),
                class_name="py-2 px-3",
              ),
              rx.el.td(
