...
def _ui_bootstrap_script() -> str:
    return (
        ...
        "if(!window.__tuwayUiObserver){"
        "window.__tuwayUiObserver=new MutationObserver(function(mutations){"
        ...
        "if((n.matches&&n.matches('.reveal,.reveal-stagger,.glow-card'))||(n.querySelector&&n.querySelector('.reveal,.reveal-stagger,.glow-card'))){"
        ...
        "if(shouldInit){init();}"
        "});"
        "window.__tuwayUiObserver.observe(document.body,{childList:true,subtree:true});"
        "}"
        "}else{"
        "init();"
        "}"
