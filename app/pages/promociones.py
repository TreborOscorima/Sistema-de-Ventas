    ...
    return rx.el.div(
        rx.el.div(
            ...
            rx.el.span(
                rx.cond(
                    p["type"] == PromotionType.PERCENTAGE,
                    p["discount_value_display"].to(str) + "% off",
                    rx.cond(
                        p["type"] == PromotionType.FIXED_AMOUNT,
                        State.currency_symbol + " " + p["discount_value_display"].to(str) + " off",
                        rx.cond(
                            p["type"] == PromotionType.BUY_X_GET_Y,
                            p["min_quantity"].to_string() + "x" + (p["min_quantity"].to(int) - p["free_quantity"].to(int)).to_string(),
                            "c/" + p["min_quantity"].to_string() + "u -" + p["discount_value_display"].to(str) + "%",
            ...
            rx.el.div(
                ...
                rx.cond(
                    ...
                    rx.el.div(
                        ...
                        rx.el.input(
                            ...
                            class_name=INPUT_STYLES["default"] + " mb-1",
                            key=State.promo_form_key.to_string(),
                        ),
                ...
                rx.el.div(
                    ...
                    rx.el.input(
                        ...
                        class_name=INPUT_STYLES["default"] + " uppercase",
                        key=State.promo_form_key.to_string(),
                    ),
                    rx.el.p(
