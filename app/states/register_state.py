...
class RegisterState(MixinState):
    ...
    def set_register_producto(self, value: str):
        if value not in ("ventas", "food", "life"):
            return
    ...
    def on_load_registro(self):
        ...
        if producto in ("food", "life"):
            self.register_producto = producto
        self.register_error = ""
    ...
    @rx.var
    def life_login_url(self) -> str:
        base = (os.getenv("PUBLIC_LIFE_URL") or "").strip().rstrip("/")
    ...
    async def handle_registration(self, form_data: dict):
        ...
        if self.register_producto == "life":
            await self._handle_life_registration(form_data)
            return
    ...
    async def _handle_life_registration(self, form_data: dict):
        """Registro de una clínica nueva en TUWAYKILIFE via HTTP.

        No escribe nada en ventas_db -- TUWAYKILIFE es un repo y una base
        de datos completamente separados, sin conexión directa. Ver
        app/services/life_api_client.py.
        """
        company_name = sanitize_name(form_data.get("company_name") or "").strip()
        email = (form_data.get("email") or "").strip().lower()
        contact_phone_country = sanitize_phone(
        ...
        if not company_name:
            self.register_error = "El nombre de la clínica es obligatorio."
            self.is_registering = False
            return
        if not email or not validate_email(email):
            self.register_error = "Ingrese un correo valido."
        ...
        from app.services.life_api_client import register_life_company

        result = await register_life_company(
            company_name=company_name,
            email=email,
            password=password,
            confirm_password=confirm_password,
