...
class DashboardState(MixinState):
    ...
    def export_categories_excel(self):
        ...
        for idx, cat in enumerate(export_categories, 1):
            ...
            # cat["total"] viene de _query_sales_by_category que ya devuelve neto de
            # descuentos Y devoluciones. Reconstruimos las columnas intermedias:
            net_sales = round(float(cat.get("total", 0)), 2)          # Venta Neta
            devolucion = round(refund_by_cat.get(cat_name, 0.0), 2)
            subtotal = round(net_sales + devolucion, 2)               # SaleItem.subtotal (pre-dev)
            disc = round(discount_by_cat.get(cat_name, 0.0), 2)
            gross = round(subtotal + disc, 2)                         # precio_lista × qty
