...
class HistoryMixin:
        ...
        with rx.session() as session:
            ...
            for log, username in results:
                entry: CashboxLogEntry = {
                    ...
                    "user": username,
                    "opening_amount": self._fmt_amount(float(log.amount or 0)) if log.action == "apertura" else self._fmt_amount(0),
                    "closing_total": self._fmt_amount(float(log.amount or 0)) if log.action == "cierre" else self._fmt_amount(0),
                    "totals_by_method": [],
                    "notes": log.notes,
                    "amount": log.amount,
                    "quantity": 1.0,
                    "unit": "",
                    "cost": log.amount,
                    "formatted_amount": self._fmt_amount(float(log.amount or 0)),
                    "formatted_cost": self._fmt_amount(float(log.amount or 0)),
    ...
    def _cashbox_sale_row(self, sale: Sale, user: UserModel | None, pm_names: dict[int, str] | None = None, log_pm_names: dict[int, str] | None = None, refund_by_sale: dict[int, float] | None = None) -> CashboxSale:
            ...
            items.append(
                {
                    ...
                    "price": self._fmt_amount(float(item.unit_price or 0)),
                    "sale_price": self._fmt_amount(float(item.unit_price or 0)),
                    "subtotal": self._fmt_amount(float(item.subtotal or 0)),
                }
            )
        ...
        sale_dict: CashboxSale = {
            ...
            "amount": self._fmt_amount(paid_total),
            "total": self._fmt_amount(float(total_amount or 0)),
            ...
            "refund_amount": self._fmt_amount(refund_amount_val),
            "net_amount": self._fmt_amount(max(net_amount_val, 0.0)),
