    ...
    def confirm_sale(self):
        ...
        with rx.session() as session:
            ...
            for item in decimal_snapshot:
                ...
                product = session.exec(
                    select(Product).where(Product.description == description).with_for_update()
                ).first()
                if not product:
            ...
            payment_label, payment_breakdown = self._payment_label_and_breakdown(sale_total_display)
            ...
            if reservation_balance > Decimal("0.00"):
                ...
                reservation.paid_amount = _round_money(reservation.paid_amount + applied_amount)
