...
| DT-4 | ✅ **Limpieza del `_vendor` muerto + unificación del mecanismo del core (CERRADO + DEPLOYADO, 2026-08-10)** | Detectado al cerrar DT-2: `_vendor` muerto en FOOD (`1e1b482`) + mecanismo inconsistente (SHOP vendor copiado vs FOOD/LIFE GitHub SHA). | ✅ **Cerrado**: unificado a **GitHub SHA vía `requirements.txt`** (core es público → sin auth), sin `_vendor`. **SHOP** (PR #16, merge `801a47a`): `git` en builder del Dockerfile (pip git+https lo requiere), core en `requirements.txt`, quitado `COPY _vendor`/install-vendor y `ensure_vendor()` de `deploy-prod.sh`; deployado (3 dominios ok, primer run del pipeline nuevo OK). **FOOD** (PR #5, merge `ea74f39`): borrado `_vendor` muerto + línea Docker redundante; deployado (`food` ok). **LIFE** ya usaba GitHub SHA (vía `requirements.lock`). Validado: build local de los 3 servicios SHOP + import `tuwayki_core`. Dev local SHOP sigue editable (`pip install -e ../tuwayki-core`). Bump del core ahora = editar el SHA en `requirements.txt` de cada repo. |
