...
| DT-4 | ⬜ **Limpieza del `_vendor` muerto de FOOD + unificar mecanismo de instalación del core** | Detectado al cerrar DT-2 (2026-08-10). (1) `Sistema-para-Food/_vendor/tuwayki-core` está en `1e1b482` (release inicial) pero **está muerto**: el `Dockerfile` de FOOD instala el core desde GitHub `@ef852f2`, NO del vendor → el directorio confunde y puede engañar en futuros diagnósticos. (2) **Mecanismo inconsistente en la flota:** SHOP instala el core desde `_vendor/tuwayki-core` copiado (Docker) + GitHub SHA (CI); FOOD y LIFE lo instalan desde GitHub SHA. El SHA ya está alineado (`ef852f2`); lo que falta es unificar CÓMO se instala. | **Follow-up futuro, no urgente** (nada roto). Opciones: borrar el `_vendor` muerto de FOOD; y decidir un único mecanismo para los 3 (todo vendor, o todo GitHub SHA) para que el diagnóstico y el bump del core sea uniforme. Hacer en su propia tarea, con re-test de cada sistema (regla de oro). |
